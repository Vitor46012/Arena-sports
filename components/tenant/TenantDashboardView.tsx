...
export default function TenantDashboardView() {
  ...
  // RTMP Local Config State
  const [rtmpUrl, setRtmpUrl] = useState('rtmp://a.rtmp.youtube.com/live2');
  const [rtmpKey, setRtmpKey] = useState('live_789456123_abc456def');
  ...
  const handleManualClip = () => {
    const result = triggerManualClip();
    showToast(`Corte manual gravado e enviado ao S3: ${result.clipName}.mp4`, 'success');
  };
        ...
        <div className="p-5 bg-slate-900 rounded-xl border border-slate-800 flex flex-col justify-between space-y-4 shadow-xl">
          ...
          <div className="space-y-3.5 flex-1">
            ...
            <button
              ...
              className="w-full py-2.5 bg-slate-800 hover:bg-slate-700 text-slate-200 border border-slate-700 rounded-lg text-xs font-bold uppercase tracking-wider transition-colors"
            >
              Salvar Parâmetros RTMP
            </button>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 pt-2">
        ...
        <div>
          <button
            ...
            className="w-full h-full min-h-[56px] py-3.5 px-4 bg-slate-900 hover:bg-slate-800 active:scale-[0.99] text-slate-200 border border-slate-800 hover:border-slate-700 rounded-xl text-xs font-bold uppercase tracking-wider transition-all flex items-center justify-center gap-2 shadow-lg"
          >
            <span className="material-symbols-outlined text-orange-500 text-[20px]">
              video_camera_front
            </span>
            <span>Gravar Lance Manual (30s)</span>
          </button>
        </div>
      </div>
